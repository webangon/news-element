...
class News_Element_Nav_Walker extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$buildercontent = $this->getItemBuilderContent( $item->mwtmpl );
		$indent         = ( $depth ) ? str_repeat( "\t", $depth ) : '';

		$class_names = $value = '';
		$classes     = empty( $item->classes ) ? array() : (array) $item->classes;
		if ( $item->nwmega == '1' ) {
			$classes[] = 'news_element_mega_menu';
		} 
		...
		if ( $item->nwmega == '1' ) {
			...
			$item_output .= sprintf( '<div class="xlmegamenu-content-wrapper" %1s>%2s</div>', '', $buildercontent );
		}
