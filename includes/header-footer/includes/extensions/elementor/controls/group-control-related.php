...
class Group_Control_Related extends Group_Control_Query {
	...
	protected function init_fields_by_name( $name ) {
		...
		$fields['post_type']['options']['related'] = __( 'Related', 'news-element' );
		...
		$related_fallback = [
			'label' => __( 'Fallback', 'news-element' ),
			'type' => Controls_Manager::SELECT,
			'options' => [
				'fallback_none' => __( 'None', 'news-element' ),
				'fallback_by_id' => __( 'Manual Selection', 'news-element' ),
				'fallback_recent' => __( 'Recent Posts', 'news-element' ),
			],
			'default' => 'fallback_none',
			'description' => __( 'Displayed if no relevant results are found. Manual selection display order is random', 'news-element' ),
			'condition' => [
				'post_type' => 'related',
			],
			'separator' => 'before',
