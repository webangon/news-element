...
class Locations_Manager {
		...
		if ( $post_id ) {
			...
			if ( $document ) {
				...
				if ( $location_settings && ! $location_settings['edit_in_content'] ) {
					$content = '<div class="elementor-theme-builder-content-area">' . __( 'Content Area', 'news-element' ) . '</div>';
				}
			}
		}
	...
	public function register_core_location( $location, $args = [] ) {
		if ( ! isset( $this->core_locations[ $location ] ) ) {
			...
			wp_die( esc_html( sprintf( __( 'Location \'%s\' is not a core location.', 'news-element' ), $location ) ) );
		}
	...
	private function set_core_locations() {
		$this->core_locations = [
			'header' => [
				'is_core' => true,
				'public' => false,
				'label' => __( 'Header', 'news-element' ),
				'edit_in_content' => false,
			],
			'footer' => [
				'is_core' => true,
				'public' => false,
				'label' => __( 'Footer', 'news-element' ),
				'edit_in_content' => false,
			],
			'archive' => [
				'is_core' => true,
