...
class Conditions_Manager {
	...
	public function admin_columns_headers( $posts_columns ) {
		...
		$posts_columns = array_slice( $posts_columns, 0, $offset, true ) + [
			'instances' => esc_html__( 'Instances', 'news-element' ),
	...
	public function admin_columns_content( $column_name, $post_id ) {
		...
		if ( ! empty( $instances ) ) {
			...
			echo esc_html__( 'None', 'news-element' );
		}
	}
	...
	public function ajax_check_conditions_conflicts( $request ) {
		...
		return esc_html__( 'Elementor recognized that you have set this location for other templates: ', 'news-element' ) .
			' ' .
			implode( ', ', $conflicted );
	}
