    ...
    class Thepack_Kit_Ajax_Manager
    {
	    ...
	    public function handle_ajax_request() {
		    if ( ! $this->verify_request_nonce() ) {
			    ...
			    $this->add_response_data( false, esc_html__( 'Token Expired.', 'news-element' ) )
			         ->send_error( 401 );
		    }

		    if(empty($_REQUEST['actions'])){
			    $this->add_response_data( false, esc_html__( 'Action not found.', 'news-element' ) )
			         ->send_error( 401 );
		    }
		    ...
		    foreach ( $this->requests as $id => $action_data ) {
			    ...
			    if ( ! isset( $this->ajax_actions[ $action_data['action'] ] ) ) {
				    $this->add_response_data( false, esc_html__( 'Action not found.', 'news-element' ), 400 );
